...
def create_stability_pool_information(SITE_ID, stabilityPoolVstBalance, stabilityPoolGemBalance,bprotocolVstBalance, bprotocolGemBalance):
    data = {}
                ...
                for index, row in users_data.iterrows():
                    if row["COLLATERAL_" + base_to_simulation] > 0:
                        ...
                        if not math.isnan(cc):
                            current_debt += cc
                max_debt = borrow_caps[base_id_to_simulation] * 2
                step_size = (max_debt - current_debt) / 10
                new_c["collaterals"] = [(current_debt + step_size * i) / ETH_PRICE for i in range(10)]
                data[key] = new_c

                stability_pool_initial_balance = stabilityPoolVstBalance[base_id_to_simulation]
                new_c["stability_pool_initial_balances"] = [
                    1 * stability_pool_initial_balance / ETH_PRICE,
                    0.5 * stability_pool_initial_balance / ETH_PRICE,
                    2 * stability_pool_initial_balance / ETH_PRICE]
...
if __name__ == '__main__':
    ...
    del inv_names["GLP"]
    base_runner.create_current_simulation_risk(SITE_ID, ETH_PRICE, users_data, assets_to_simulate, assets_aliases,collateral_factors, inv_names, liquidation_incentive, total_jobs,True)
