...
def compare_to_prod_and_send_alerts(name, base_SITE_ID, current_SITE_ID, bot_id, chat_id, slippage_threshold=5,
    ...
    for key1 in prod_file:
        ...
        for key2 in prod_file[key1]:
            ...
            if abs(change) > slippage_threshold:
                ...
                message = f"{time_from_prod} {name}.{key1}.{key2}  Liquidity Change (%) {round(change, 2)} " \
    ...
    for market in oracle_file:
        ...
        if abs(diff) > 3:
            message = f'{time_from_prod} {name}.{market}.oracle.dex Price is off by: {round(diff, 2)} Oracle Price: {oracle} Dex Price: {dex}'
            print(message)
            alert_sent = True
            if send_alerts:
                print("Sending To TG")
        ...
        if cex:
            ...
            if abs(diff) > 3:
                message = f'{time_from_prod} {name}.{market}.oracle.cex Price is off by: {round(diff, 2)} Oracle Price: {oracle} Cex Price: {cex}'
                print(message)
                alert_sent = True
                if send_alerts:
                    print("Sending To TG")
