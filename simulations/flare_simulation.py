...
def run_single_simulation(date_file_name,
                          ...
                          min_usd_cr, safe_usd_cr, usd_collateral_ratio,
                          usd_dl_x, flr_dl_x,
    ...
    try:
        ...
        for row in file:
            ...
            if usd_ucr <= min_usd_cr \
                ...
                state = 1
                l_size = usd_liquidation_size_with_flare(safe_usd_cr, row_btc_usd_price, usd_collateral_volume, debt_volume,
                ...
                obj = uni_box.find_btc_liquidation_size(burned_btc_volume, flr_liquidation_volume, usd_liquidation_volume)
        ...
        time_series_report_name = f"webserver\\flare\\" + SITE_ID + "\\" \
                                                                    f"BtcUsdStd-{btc_usd_std}+" \
                                                                    f"FlareBtcStd-{flr_btc_std}+" \
                                                                    ...
                                                                    f"SafeUsdCr-{safe_usd_cr}+" \
                                                                    f"MinFlareCr-{min_flare_cr}+" \
                                                                    f"SafeFlareCr-{safe_flare_cr}+" \
        ...
        x1 = ax1.plot(report_df["timestamp"], report_df["btc_usd_price"] / report_df["btc_usd_price"].max(), 'r-', label="Btc Usd Price")
        x2 = ax1.plot(report_df["timestamp"], report_df["flare_btc_price"] / report_df["flare_btc_price"].max(), 'r-', label="Flare Btc Price")
        x3 = ax2.plot(report_df["timestamp"], report_df["usd_ucr"], label="Usd CR")
        x4 = ax2.plot(report_df["timestamp"], report_df["flare_ucr"], label="Flare CR")
        x5 = ax2.plot(report_df["timestamp"], report_df["debt_volume"] / report_df["simulation_initial_debt_volume"], label="DebtVolume")
        x6 = ax2.plot(report_df["timestamp"], report_df["usd_collateral_volume"] / pow(report_df["simulation_initial_debt_volume"], 2), label="UsdCollateralVolume")
        x7 = ax2.plot(report_df["timestamp"], report_df["flare_collateral_volume"] / pow(report_df["simulation_initial_debt_volume"], 2), label="FlareCollateralVolume")
        x8 = ax2.plot(report_df["timestamp"], report_df["open_liquidation"] / report_df["simulation_initial_debt_volume"], label="OpenLiquidations")
        x9 = ax2.plot(report_df["timestamp"], report_df["total_liquidations"] / report_df["simulation_initial_debt_volume"], label="TotalLiquidations")
        ...
        return {"btc_usd_std": btc_usd_std,
                ...
                "min_usd_ucr": min_usd_ucr,
                "min_flare_ucr": min_flare_ucr}


def run_simulation(c, simulation_file_name):
    summary_report = []
    for r in itertools.product(c["btc_usd_std"], c["flare_btc_std"],
                               c["debt_volume"], c["usd_volume"], c["flare_volume"],
                               c["min_usd_cr"], c["safe_usd_cr"], c["usd_collateral_ratio"],
                               c["usd_dl_x"], c["flare_dl_x"],
                               c["usd_dl_recovery"], c["flare_dl_recovery"],
                               c["min_flr_cr"], c["safe_flr_cr"]):
...
c = {
    ...
    "usd_volume": [initial_dept_volume * 1.4],
    "flare_volume": [initial_dept_volume * 2],
    "usd_dl_x": [1_000_000],
    "usd_dl_recovery": [30],
    "flare_dl_x": [1_000_000],
    "flare_dl_recovery": [30],
    "min_usd_cr": [1.2],
    "safe_usd_cr": [1.4],
    "min_flare_cr": [1.5],
    "safe_flare_cr": [2],
    "usd_collateral_ratio": [0.8]}
