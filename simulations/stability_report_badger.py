...
class stability_report:
    ...
    def plot_for_html(self, directory, name, print_time_series, li):
        ...
        for index, row in uniques.iterrows():
            ...
            plt.suptitle(
                "Dex Liquidity (M-USD): " + str(row["volume_for_slippage_10_percents"] * self.ETH_PRICE / 1_000_000))
    ...
    def run_simulation(self, output_directory, file_name, name, config, print_time_series, liquidation_df, skip,
        ...
        try:
            ...
            total_runs = len(config["volume_for_slippage_10_percentss"]) * len(config["l_factors"]) * len(
                config["price_recovery_times"]) \
                         * len(config["share_institutionals"]) * len(config["recovery_halflife_retails"]) * len(
                config["collaterals"]) \
            ...
            for volume_for_slippage_10_percents in config["volume_for_slippage_10_percentss"]:
                base_volume_for_slippage = volume_for_slippage_10_percents
